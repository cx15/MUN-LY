  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      if (path === "/api/submit-conference" && request.method === "POST") {
        return await handleConferenceSubmission(request, env2);
      }
...
async function handleConferenceSubmission(request, env2) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization"
  ...
  try {
    ...
    await sendEmailNotification(data, env2);
    return new Response(JSON.stringify({
      success: true,
    ...
    console.error("Conference submission error:", error3);
    return new Response(JSON.stringify({
...
__name(handleConferenceSubmission, "handleConferenceSubmission");
async function sendEmailNotification(data, env2) {
  try {
...
Submitted at: ${(/* @__PURE__ */ new Date()).toISOString()}
    ...
    console.error("Email notification error:", error3);
  }
}
